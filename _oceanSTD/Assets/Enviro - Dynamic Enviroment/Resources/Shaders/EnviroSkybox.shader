...
#include "UnityCG.cginc"
	...
	float4 frag(v2f i) : SV_Target
	{
		...
		//half3 moonNormal = UnpackNormal(tex2D(_MoonNormal, i.moonPos.xy ));
		//float d = saturate(max(0.0,dot(moonNormal, lightVector)) * 4);
		//moonSampler = lerp(float4(0,0,0,0),moonSampler,d);
		float3 moonArea = clamp(moonSampler * 100 ,0,1);
		moonSampler = (moonSampler * _MoonColor) * 2;
		//float3 moonArea = pow(dot(viewDir, -_MoonDir), (21.0 - _MoonSize) * 20);
